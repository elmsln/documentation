# ELMSLN Vagrant Instructions {#elmsln-vagrant-instructions}
...
### How to use this to bring up ELMSLN {#how-to-use-this-to-bring-up-elmsln}
...
4. Open Terminal\(OSX/Linux\) or PowerShell \(Windows\) and navigate to the folder you want elmsln in. Now clone the repository using the command \(
   `git clone https://github.com/elmsln/elmsln.git`
   \)
   ![](https://cloud.githubusercontent.com/assets/16597608/13260179/60875b1e-da28-11e5-865f-89e6586f370a.PNG "clone elmsln")

### Spin up the development instance \(OSX / Linux\) {#spin-up-the-development-instance-osx-linux}
...
### Spin up the development instance \(Windows\) {#spin-up-the-development-instance-windows}
...
If you have issues downloading the box try running`vagrant box add --insecure -c elmsln/ubuntu16 --force`ahead of time which will download just the box in question. This will take a long time the first time you do it as it has to download a server image. After the first build, future builds from scratch should take about 10-12 minutes.

### After installation completes {#after-installation-completes}

Now you'll be able to jump into any of the domains that ELMSLN starts to establish for use! Go to http://online.elmsln.local/ after installation completes. If it all worked you should see a new Drupal site running the Course Information System \(CIS\) distribution._If this address doesn't resolve, you may need to do this extra step at the bottom in Legacy - host file management_

You can log into this with`user: admin | password: admin`

![](https://cloud.githubusercontent.com/assets/329735/22887464/ca97877e-f1c7-11e6-98df-2207a421204d.png "log in")

To connect to the console of your instance:`vagrant ssh`Here you'll have access to command line elms called`leafy`. Type`leafy`to see developer options for selection. This also comes installed with a fully primed copy of`drush`so you can type`drush sa`to see a list of possible drupal sites to run commands against.`drush @elmsln`is a target that will run the command against every site it finds in the elmsln instance.
...
### Legacy - host file management {#legacy-host-file-management}
...
### Spin up the vagrant instance\*\* {#spin-up-the-vagrant-instance}
...
![](https://cloud.githubusercontent.com/assets/16597608/13260446/767d8ae6-da29-11e5-8346-393a09c54cf6.PNG "log in")

To connect to the console of your instance:`vagrant ssh`

### Create a new course {#create-a-new-course_1}
